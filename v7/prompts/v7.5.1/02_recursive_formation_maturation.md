# V7.5.1 Turn 2: Recursive Formation Maturation
...
concise working name and carry it forward as one whole without spending its
carriers. Re-enter any carrier through another exact operation or relation and
let the same carrier belong to several independent wholes. Re-entry adds
formations without dissolving the first whole back into separate branch
observations.
...
A distributed relational resonance is a structural constellation complete at
the scale of one exact shared relation. Its lexical realizations may occupy
distinct bodies, materials, scenes, scales, or media, but each enacts the same
verb-level change through the same relational role pattern. It does not require
one literal participant system, causal process, or formal passage topology.
Each realization keeps its local medium and roles while contact among them
makes the shared transformation more exact and changes expectation elsewhere
in the passage.
...
-> one role pattern undergoes the same before-to-after change across them
-> the realizations strengthen one another after contact
-> the shared transformation changes passage perception
```

Inhabit the invariant as one relational sentence whose roles, direction, and
before-to-after change fit every realization without replacing its operative
verb. A common dimension such as access, continuity, capacity, boundary,
force, or consequence is not itself an invariant. When local realizations
require different relations, keep their attraction alive and form the more
specific complementary operations or constellations available among them.

Completion at the shared scale does not complete or consume a realization's
other roles, circuits, or consequences. Alternative outcomes may remain inside
one resonance when they branch from the same exact role pattern. When they
perform different transformations, carry them as separate formations and then
let their contact produce another consequence.
...
## Carrier re-entry, role relay, and functional branching

Temporarily follow operations and concrete entities instead of existing
formation names. Using one clause, surface occurrence, or branch facet in a
role consumes neither the carrier nor the role. Let it enter another relation,
and keep every other supplied participant, predicate, direction, phase,
outcome, number relation, and material relation available for formation.

Inhabit the counterpart or second part, source, product, measure and measured
thing, instrument, target, recipient, return path, and every other position
opened by an operation. Carry these roles forward and backward through the
exact passage. A passage relation may fill a role, exchange or reverse its
positions, or make its continued absence the operative change.

When the same concrete entity, act, or technical relation appears in separate
branch descriptions, keep it continuous as its role changes and inhabit the
resulting relay. When an exact surface act or grammatical relation is named
inside another carrier's branch prose, form that surface-to-branch circuit in
its own right, even when the same carriers already sustain another governing
circuit.

When one concrete configuration changes function under different surrounding
relations, give each function a complete recursive trajectory before bringing
them together. Let each recruit the whole supplied field, alter its earlier
carriers, and change passage expectation. Carry both acquired consequences at
the force they gain through whole-field incorporation.

After a distributed transformation becomes visible, keep it active and
re-enter each exact realization outside that invariant. Follow any distinct
operation, role relay, circuit, or consequence the realization has acquired
through contact with the whole. A realization may remain inside the
distributed resonance and simultaneously carry another operational
configuration or structural constellation.
...
- an exact surface act or grammatical relation named again inside supplied
  branch prose;
- the same concrete entity recurring across branch descriptions with a changed
  role;
...
Hold the resulting shared transformation as an additive overlay. Re-enter its
realizations through their unused roles and through every more specific
relation among a subset of carriers. The overlay may strengthen those
formations and be strengthened by them; it never replaces them with its common
dimension.
...
Preserve formation at every scale that has acquired a distinct change in
passage perception. A nested operation or exact carrier subset does not become
an example of a broader resonance merely because both use the same material.
...
When one carrier belongs to a distributed overlay and another exact formation,
make both memberships and both passage changes recoverable. When one concrete
configuration has acquired opposed functions, preserve both complete
trajectories and the consequence of their interaction.
