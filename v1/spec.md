...
```text
Use your previous synthesis, and any coverage appendix or source notes you produced, as the main basis, checking the permitted source findings selectively for any finding that is omitted, generic, under-specified, or not clearly assigned a role. Pay particular attention to distinctive lexical images that were technically mentioned yet hidden beneath generic wording. Restore every strong or medium-grade finding whose specific mechanism has been underemphasized, giving it an audible function inside the larger passage-scale synthesis rather than necessarily its own sentence or paragraph; retain weak findings whenever they meaningfully complete that synthesis, while preserving their evidential limits. Merge near-findings that perform parts of the same mechanism, and let several textual anchors work within one scene whenever they advance the same relational movement. Split a scene when clarity requires it, carrying the active image continuously forward. While recovering hidden mechanisms, merge paragraphs that restate the same relation, absorb grammatical qualifications and epistemic limits into the scene they serve, and reserve the complete backward replay for the final ending. Then rewrite the entire synthesis in fluent, beautiful Turkish prose—not as corrections, annotations, or a branch catalog—so that each recovered element arises naturally at its point in the Quranic sequence, performs a clear role, uses the later corroboration already recorded in the sources, and remains grounded in the word’s primary contextual meaning while adding its secondary geometry. Keep the reader oriented in the passage as the synthesis progressively deepens, use only the sacred Arabic text and supplied finding files, and make the revised prose stand entirely on its own.
```
