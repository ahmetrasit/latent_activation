...
The renderer uses exact Arabic script selectively for decisive Quranic surface words from the passage. Every Arabic span is copied as an exact contiguous substring of the supplied passage text, without normalization, shortening, reconstruction, or loss of attached particles and endings. It does not speak bare spaced roots, recite branch inventories, or introduce non-passage branch forms merely to prove evidence. Root relations are explained in ordinary Turkish as relationships within a word family. Turkish carries the complete explanation, and Arabic supplies limited, repeated familiarity with the words the listener actually hears in the Quran.

The prose creates movement through its objects, actions, returns, contrasts, and changes of scale. Lexical provenance and mechanism-map taxonomy remain in silent reasoning and grade metadata rather than being narrated through labels for word families, semantic fields, branches, domains, evidence distance, activation, or confidence. When a lexical relationship must be explicit, the renderer connects the Quranic surface word to its concrete image once and then lets objects, actions, positions, and consequences carry the finding. It does not cycle through synonymous labels to disguise repetition, and it favors immediately clear everyday Turkish over rare specialist or theological loanwords. The opening follows the primary scaffold closely for names, titles, roles, and direct propositions rather than replacing them with more interpretive synonyms. The prose never exposes production language such as camera, frame, shot, zoom, or close-up. It sounds conversational and human without becoming a sermon, classroom lesson, announcer script, or imitation of a named speaker.
