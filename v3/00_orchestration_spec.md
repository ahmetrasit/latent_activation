...
Arabic passage tokens, roots, lemmas, and branch forms remain in Arabic script; the renderer does not replace them with Turkish or Latin transliteration. The reader is assumed to know no Arabic: Arabic script serves only as an orthographic anchor, and every sentence must remain understandable when it is skipped. Each Arabic form therefore receives a clear Turkish meaning or function in the same sentence, with an immediate gloss at first mention. Ordinary Turkish explanation may use established Arabic-origin Turkish vocabulary, but a lexical reference uses the Arabic form already present in the listed inputs, such as `ٱلصِّرَٰطَ` (yol), `ق و م` (dosdoğru) kökü, or `القامة` (makara).
