...
ROOT_MAPPING_REPAIRS = WORKFLOW_ROOT / "data" / "root_mapping_repairs.json"
...
def load_root_mappings(
    ...
    with tempfile.NamedTemporaryFile(suffix=".sqlite") as temp_db:
        ...
        connection.close()
    apply_root_mapping_repairs(mappings)
    return mappings


def apply_root_mapping_repairs(mappings: dict[str, dict[str, Any]]) -> None:
    """Apply only reviewed missing mappings; never replace a mapped/split root."""
    repairs = json.loads(ROOT_MAPPING_REPAIRS.read_text(encoding="utf-8"))
    for repair in repairs["repairs"]:
        record = mappings.get(repair["qac_root"])
        if record is None or record["targets"]:
            continue
        for field, expected in repair["expected_mapping"].items():
            if record.get(field) != expected:
                raise ValueError(f"reviewed mapping precondition changed: {repair['id']}/{field}")
        record.update(
            mapping_status="reviewed_occurrence_alignment",
            matched_occurrences=1,
            unmapped_reason="",
            targets=[copy.deepcopy(repair["target"])],
        )


def validate_repair_occurrences(ayat: list[dict[str, Any]], mappings: dict) -> None:
    """Do not apply the reviewed identity to a changed QAC occurrence."""
    repairs = json.loads(ROOT_MAPPING_REPAIRS.read_text(encoding="utf-8"))
    for repair in repairs["repairs"]:
        record = mappings.get(repair["qac_root"], {})
        if record.get("mapping_status") != "reviewed_occurrence_alignment":
            continue
        for ayah in ayat:
            for occurrence in ayah.get("root_occurrences", []):
                if occurrence["root"] != repair["qac_root"]:
                    continue
                expected = repair["packet_occurrence"]
                if ayah["ref"] != expected["ref"] or any(
                    occurrence.get(key) != expected[key]
                    for key in ("word_indices", "surfaces_ar", "lemmas_ar")
                ):
                    raise ValueError(f"reviewed QAC occurrence changed: {repair['id']}")
...
def main() -> None:
    ...
    validate_repair_occurrences([*ayat, *out_of_window_ayat], root_mappings)
    ...
    resource_hashes = {
        ...
        for path in (args.qac, args.branches, args.root_map, quran_path, ROOT_MAPPING_REPAIRS)
    }
    rootless_ayat = [
        {
            "ref": ayah["ref"],
