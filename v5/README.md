# GSLS-1A 5.1 — One Author, Activation-Centered Synthesis
...
4. `04_publication_continuation.md` — publication prose by the same author, with a paragraph eligibility test that rejects ayah-by-ayah or section-by-section summary writing.

There is no critic, audit agent, adjudicator, manifest editor, semantic QC agent, or separate renderer.

Furūq v4 semantic access is limited to `branch_image_ar` and `what_is_ar`. `root_norm` and `branch_id` are lookup identifiers only.
