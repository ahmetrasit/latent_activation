# V8.4 Cold Orchestration Specification

This file is the executable orchestration contract for running V8.4 on one
surah from a cold context. It tells the orchestrator how to select inputs,
create the run, isolate production agents, sequence the five turns, and hand
off the resulting artifacts.

The five files under `v8.4/prompts/` are authoritative for semantic behavior.
...
For orchestration, this file governs if `v8.4/README.md`, an older run task, or
...
One surah uses two production roles and five prompt turns:
...
Agent 2 — one fresh persistent session
  Turn 4: compositional semantic synthesis
  Turn 5: post-draft recomposition
```
...
surah's agent. Save its returned agent/session identifier after Turn 4 and
deliver Turn 5 as a follow-up task to that same identifier. A fresh author
revising another author's draft does not satisfy the post-draft design.
...
Do not copy or transform the bundle. V8.4 reads the existing input materials in
place.

## 5. Create a non-destructive run root

Unless the user supplies a run label, use `pilot`. Build the run root as
`v8.4/run/sNNN-<label>-YYYYMMDD/` using the local calendar date. The default is:

```text
v8.4/run/sNNN-pilot-YYYYMMDD/
```
...
The five canonical artifact paths are:

```text
a1/01-branch-seeded-formations-v8.4.md
a1/02-recursively-expanded-field-v8.4.md
a1/03-reciprocal-formation-field-v8.4.md
a2/04-compositional-semantic-synthesis-v8.4.md
a2/05-postdraft-recomposition-v8.4.md
```
...
|---|---|---|---|---|---|
| 1 | persistent Agent 1 | `01_branch_seeded_formation-v8.4.md` | yes | no | none |
| 2 | same Agent 1 | `02_recursive_reopening-v8.4.md` | yes | no | Turn 1 |
| 3 | same Agent 1 | `03_reciprocal_scaffold_integration-v8.4.md` | yes | yes, first exposure | Turns 1–2 |
| 4 | fresh Agent 2 | `04_compositional_semantic_synthesis-v8.4.md` | yes | yes | Turns 1–3 |
| 5 | same Agent 2 | `05_postdraft_recomposition-v8.4.md` | yes | yes | Turns 1–4 |
...
- `v8.4/README.md` or this orchestration specification;
- any other V8.4 run;
...
separate assessment. Such material must never be fed back into the five
production turns.
...
```text
You are the persistent V8.4 formation author for Surah <N>. This is Turn 1 of
three turns in the same session.

Read `v8.4/prompts/01_branch_seeded_formation-v8.4.md` completely and follow it.
...
Do not read documentation, prior V3/V7/V8.4 analyses, publications, reference
outputs, or any other passage material. Work only inside the authorized closed
field.

Write the artifact with apply_patch to:
<RUN_ROOT>/a1/01-branch-seeded-formations-v8.4.md
...
Read `v8.4/prompts/02_recursive_reopening-v8.4.md` completely and follow it.
...
Write the artifact with apply_patch to:
<RUN_ROOT>/a1/02-recursively-expanded-field-v8.4.md
...
Read `v8.4/prompts/03_reciprocal_scaffold_integration-v8.4.md` completely and follow
it.
...
Write the artifact with apply_patch to:
<RUN_ROOT>/a1/03-reciprocal-formation-field-v8.4.md
...
```text
You are a fresh independent V8.4 synthesis author for Surah <N>. You did not
write the formation artifacts and have no cross-passage context.

Read `v8.4/prompts/04_compositional_semantic_synthesis-v8.4.md` completely and follow it.
...
Write the artifact with apply_patch to:
<RUN_ROOT>/a2/04-compositional-semantic-synthesis-v8.4.md
...
non-empty. Save Agent 2's identifier; the first composition is not yet the
final V8.4 artifact. Do not assess its content or steer the author before the
follow-up.

## 12. Turn 5 — follow up with the same Agent 2

Send a follow-up task to the saved Agent 2 identifier. Do not spawn another
worker and do not summarize what the first draft missed.

```text
Continue the same persistent Surah <N> synthesis session with Turn 5 only.
You are revising the complete synthesis you wrote in Turn 4.

Read `v8.4/prompts/05_postdraft_recomposition-v8.4.md` completely and follow it.
...
Authorized first composition:
- <RUN_ROOT>/a2/04-compositional-semantic-synthesis-v8.4.md

Do not read documentation, prior V3/V7/V8.4 analyses, publications, reference
outputs, other surahs, or outside material. Do not use comparison feedback or
a preselected list of recoveries. Let the prompt and supplied field determine
whether and how the composition changes.

Write the self-standing final artifact with apply_patch to:
<RUN_ROOT>/a2/05-postdraft-recomposition-v8.4.md

Do not put process commentary, a revision report, or self-assessment in the
artifact. On completion, report only the path and a concise structural count.
```

Wait for completion and confirm only that the expected artifact exists and is
non-empty. Do not accept a residual-findings appendix in place of the requested
self-standing recomposition; content quality remains a post-run assessment.

## 13. Minimal completion contract

The production run is complete when all five conditions hold:

1. The three Agent 1 artifacts originated from one persistent session across
   Turns 1–3.
2. Agent 1 first saw the scaffold in Turn 3.
3. Agent 2 was a fresh, non-forked worker at Turn 4.
4. The same Agent 2 authored Turn 4 and recomposed it in Turn 5.
5. All five canonical artifacts exist at the selected run root.

Report the run root and five artifact paths. Also report any model, context,
...
author into the old reservoir and call it an exact V8.4 run. Start a new Agent 1
from Turn 1 in a new run root, or clearly label the run as a procedural
deviation.

If Agent 2 fails during Turn 4 and its expected path is absent, a newly spawned
cold replacement may repeat Turn 4 from the same authorized inputs because the
synthesis role is intentionally fresh at entry. That replacement must then
continue through Turn 5 in its own session.

If Turn 4 is complete and Agent 2 fails during Turn 5 while its session remains
usable, send a concise follow-up asking it to finish Turn 5 at the same path.
Do not supply a diagnosis or recovery list. If the session is lost, do not ask a
new author to recompose the old author's Turn 4 and call it an exact run. Start
the synthesis role again in a new run root: preserve the completed Agent 1
reservoir unchanged, spawn a fresh Agent 2, and have that author perform both
Turns 4 and 5.
...
Agent 2 against that unchanged reservoir for Turns 4–5. Report the restart.
The copy is an orchestration recovery operation, not a new semantic turn.
...
## 16. Post-run assessment, when requested

Assessment is separate from production and is performed by the orchestrator,
not by either production agent. Read the five completed artifacts directly.
Compare Turn 4 with Turn 5 before opening any previous synthesis or benchmark
assessment. Previous V3, V7, S1, or other V8.4 outputs may be used only after
...
- Did the synthesis preserve precise lexical surprises that make surface wording
  sound different?
- Did Turn 5 recover a consequential formation or discover an exact
  formal-lexical relation that Turn 4 did not carry?
- Did independently reinforcing material change the basis, scope, mechanism,
  participant structure, boundary, or embodiment of an existing result rather
  than merely adding another image?
- Did recovery weaken an exact lock, erase a participant distinction, or
  flatten unequal formations into coequal sections?
- Did Turn 5 make unnecessary changes when Turn 4 was already complete?
...
## 17. Parallel orchestration of several surahs
...
- keep that same Agent 2 through Turns 4 and 5;
- keep every artifact path passage-specific.

Parallelism changes scheduling only. It does not change the five-turn semantic
