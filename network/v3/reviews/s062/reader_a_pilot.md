...
- Semantic invariant: A message becomes transmissible when it is externalized in a medium that can preserve sequence, whether a responsive chain of voices or a materially joined and opened record.
...
- Surprising reach: Recitation and scripture expand into two concrete communication systems: an acoustic relay with range and response, and a crafted document whose parts are joined, registered, and unfolded.
...
- Semantic invariant: Value can be negotiated through exchange and price or received as allotted and surplus benefaction; the two economies compete for practical allegiance.
...
- Surprising reach: The market is not a generic temptation but a complete transactional mechanism, set against provision that combines assigned share with unowed excess.
...
- Semantic invariant: A declaration becomes accountable when it can be tested by fulfilled action or solemnized as a pledge that places trust and liability behind the words.
- Surface relation: direct; 62:5 names denial, 62:6 frames an asserted privilege as a conditional truth claim and commands a wish for death, and 62:7 exposes the refusal to perform that speech.
- Surprising reach: The challenge behaves like a speech-act test: claim, oath, surety, hand-pledge, wish, and enacted truth converge on whether words can bear their promised consequence.
...
- Semantic invariant: Avoidance cannot prevent a destined encounter; the fugitive is returned into a field where concealed and present realities are equally knowable and deeds become reportable evidence.
...
- Surprising reach: Eschatological return is staged as pursuit, courtroom disclosure, and renewed standing, making hidden action materially catch up with the one who tried to escape.
...
- Semantic invariant: Effective authority sustains a polity, restrains deviation, and converts measured judgment into an order that actually takes effect.
...
- Surprising reach: Wisdom is materialized as a bridle, sovereignty as a supporting hand and mainstay, and decree as fabrication whose precision makes an intended form hold.
