...
sources. Do not consult `label_hint` until the branch-first motif and scene
...
Judge by branch-image coherence, surface context, root/ayah coverage, and
...
1. Atomic motif: one precise image, action, role, object, or relation.
2. Subchannel: a coherent scene, process, or social frame supported by several
   atomic motifs.
...
1. Extract before clustering. Without consulting `label_hint`, inventory atomic
   motifs from branch images, `what_is_ar`, construction edges, and surface
   context. Do not start by naming broad domains.
2. Assign every atomic motif a semantic role: object, component, material,
   action, setting, agent, state, relation, or outcome. Use more than one role
   only when the branch evidence requires it.
...
4. Follow shared branch hinges and connected construction edges across rows.
   A coherent scene or mechanism may be distributed across several families;
   no single row needs to contain every component. A complete multi-part scene
   can be meaningful even when it appears only once.
5. Build subchannels with the one-scene test: branches belong together when
   they can participate in one intelligible scene, process, mechanism, or
   social frame.
...
8. Do not consolidate through topical similarity alone. Keep distinct scenes,
   processes, mechanisms, or social frames separate unless a bridge explains
   their relation.
...
14. After the branch-first construction is complete, consult `label_hint` only
    to check whether a supported motif or scene was overlooked. Do not rename
    channels merely to match a hint.
15. Perform a final coverage sweep. Assign every materially supported motif to
    a parent subchannel, standalone subchannel, bridge, lexical resonance,
    surprise probe, or residual class. Reserve residual status for material
    that remains unintelligible after reading its branch evidence; weak,
    overlapping, reused, or unexpected material must remain visible elsewhere.
...
define each atomic motif once, reference it thereafter by name and branch ID,
and avoid repeating the same branch definition across sections. Do not copy
full queue rows into the report, but include enough branch, surface, and
support detail that a downstream prose agent can integrate the findings with
ayah-based commentaries without needing the original bundle.
...
branch image that materially belongs to the parent, subchannel, bridge, or
surprise probe. Do not include unrelated branch tails merely because they
...
- Nucleus coherence: do the branches describe one recognizable frame?
...
## Atomic Motif Inventory

- Motif name [semantic role; surface-primary | latent/lexical | mixed]:
  root:B### `branch_image_ar`; surface-root occurrence from `surface_context`.
...
- One-scene frame:
- Atomic motifs:
- Branch evidence: root:B### `branch_image_ar`; root:B### `branch_image_ar`
...
- Motif or row class: explain why it remains unintelligible after branch-level
  reading. Do not use residual status solely for weak, reused, overlapping, or
  unexpected evidence; preserve such material as a resonance or probe.
```
