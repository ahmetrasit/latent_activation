...
`prompt.md`, `packet.json`, `response.schema.json`, and `assignment.json`. Write
...
mapped Furuq targets, including non-dominant targets. `branch_inventory` stores
each mapped root once, shared by focus and context occurrences. Each branch's
`variants` preserves a source row's paired Arabic image and scope, English fields,
and provenance. Read the Arabic image **with its scope**; do not collapse distinct
...
the citable window. File paths in source records are provenance, not files to open.
...
Write one object matching `response.schema.json`; copy `reader_id`, `focus_ref`,
and all `input_identity` hashes from `assignment.json`. Set protocol to
`hft-v2-response-v1` and `trace_kind` to `reconstructed`.
