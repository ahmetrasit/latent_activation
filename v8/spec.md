# V8 Cold Orchestration Specification

This file is the executable orchestration contract for running V8 on one
...
The four files under `v8/prompts/` are authoritative for semantic behavior.
...
For orchestration, this file governs if `v8/README.md`, an older run task, or
...
If the platform exposes a differently named spawn tool, use it only if it can
guarantee the same four properties: worker role, no forked context,
`gpt-5.6-sol`, and maximum reasoning. Report any unavoidable deviation; do not
silently claim an exact V8 run.

Do not set a service tier unless the user separately requests one.

## 3. Architecture and session identity

One surah uses exactly two production agents and four prompt turns:
...
Agent 2 — one fresh session
  Turn 4: independent semantic atlas
```
...
4. If no full bundle exists and exactly one candidate bundle exists, use that
   candidate.
5. If selection remains ambiguous, stop and ask the user which bundle to use.
...
Do not copy or transform the bundle. V8 reads the existing input materials in
place.

## 5. Create a non-destructive run root

Unless the user supplies a run label, use `pilot`. Build the run root as
`v8/run/sNNN-<label>-YYYYMMDD/` using the local calendar date. The default is:

```text
v8/run/sNNN-pilot-YYYYMMDD/
```

Create these directories:

```text
v8/run/sNNN-pilot-YYYYMMDD/a1/
v8/run/sNNN-pilot-YYYYMMDD/a2/
```
...
```text
a1/01-branch-seeded-formations.md
a1/02-recursively-expanded-field.md
a1/03-reciprocal-formation-field.md
a2/04-independent-semantic-atlas.md
```
...
|---|---|---|---|---|---|
| 1 | persistent Agent 1 | `01_branch_seeded_formation.md` | yes | no | none |
| 2 | same Agent 1 | `02_recursive_reopening.md` | yes | no | Turn 1 |
| 3 | same Agent 1 | `03_reciprocal_scaffold_integration.md` | yes | yes, first exposure | Turns 1–2 |
| 4 | fresh Agent 2 | `04_independent_semantic_atlas.md` | yes | yes | Turns 1–3 |

For Turns 1 and 2, do not include `primary-scaffold.md` in the task message,
do not mention its contents, and explicitly prohibit reading it. The scaffold
enters Agent 1's session for the first time in Turn 3.
...
- `v8/README.md` or this orchestration specification;
- any other V8 run;
...
```text
You are the persistent V8 formation author for Surah <N>. This is Turn 1 of
three turns in the same session.

Read `v8/prompts/01_branch_seeded_formation.md` completely and follow it.
...
Do not read documentation, prior V3/V7/V8 analyses, publications, reference
outputs, or any other passage material. Work only inside the authorized closed
field.

Write the artifact with apply_patch to:
<RUN_ROOT>/a1/01-branch-seeded-formations.md
...
Read `v8/prompts/02_recursive_reopening.md` completely and follow it.
...
Write the artifact with apply_patch to:
<RUN_ROOT>/a1/02-recursively-expanded-field.md
...
Read `v8/prompts/03_reciprocal_scaffold_integration.md` completely and follow
it.
...
Write the artifact with apply_patch to:
<RUN_ROOT>/a1/03-reciprocal-formation-field.md
...
```text
You are a fresh independent V8 synthesis author for Surah <N>. You did not
write the formation artifacts and have no cross-passage context.

Read `v8/prompts/04_independent_semantic_atlas.md` completely and follow it.
...
artifacts as generative material, not a canonical hierarchy. Preserve
autonomous material centers when they change a distinct dimension; do not
subsume every formation under one relational architecture.

Do not read documentation, prior V3/V7/V8 analyses, publications, reference
outputs, other surahs, or outside material.

Write the artifact with apply_patch to:
<RUN_ROOT>/a2/04-independent-semantic-atlas.md

Do not put process commentary or self-assessment in the artifact. On
completion, report only the path and a concise structural count.
```

Wait for completion and confirm that the expected atlas exists and is
non-empty.
...
1. Agent 1 remained one session across Turns 1–3.
...
If Agent 1 fails during a turn but its session remains usable, send a concise
...
author into the old reservoir and call it an exact V8 run. Start a new Agent 1
from Turn 1 in a new run root, or clearly label the run as a procedural
deviation.

If Agent 2 fails, a newly spawned replacement Agent 2 may repeat Turn 4 from
the same authorized inputs because Turn 4 is intentionally fresh.

If any artifact path already contains uncertain or partial material, preserve
it and use a new run root rather than destructively replacing it.
...
Previous V3, V7, S1, or other V8 outputs may be used only now as comparison
material.
...
- Did the atlas preserve precise lexical surprises that make surface wording
  sound different?
