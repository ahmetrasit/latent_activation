# V8.4.5 Controlled Orchestration Specification

This is the controlling cold-run specification for V8.4.5. Before taking any
...
- substitute `v8.4.5` for `v8.4` in prompt, run, and artifact paths;
...
V8.4.5 otherwise inherits the V8.4 cold workflow, agent isolation, input
boundaries, and Turn 1–4 semantics. The versioned copies in
`v8.4.5/prompts/` keep those semantics unchanged. It preserves V8.4.2's
two-turn recovery/recomposition boundary. Turn 5 adds an exact-carrier
collision traversal and then performs a contact-type eligibility pass before
inherited-draft ranking. That pass separates direct surface locks,
independently instantiated root-operation locks, explicitly conflicting forms,
and merely self-repeating examples. Turn 6 makes eligible locks the spines of
broader mechanisms, assigns publication rank and scale by nonredundant passage
work, and calibrates source-example evidence without suppressing it. After
forming a provisional final, Turn 6 must complete the separate evidence-form,
morphology-entailment, and consequence-survival passes before writing the
...
```text
v8.4.5/prompts/01_branch_seeded_formation-v8.4.5.md
v8.4.5/prompts/02_recursive_reopening-v8.4.5.md
v8.4.5/prompts/03_reciprocal_scaffold_integration-v8.4.5.md
v8.4.5/prompts/04_compositional_semantic_synthesis-v8.4.5.md
v8.4.5/prompts/05_postdraft_recovery_field-v8.4.5.md
v8.4.5/prompts/06_selective_final_recomposition-v8.4.5.md
```

Write every generated artifact with `v8.4.5` in its filename. The synthesis
paths are:

```text
a2/04-compositional-semantic-synthesis-v8.4.5.md
a2/05-postdraft-recovery-field-v8.4.5.md
a2/06-selective-final-recomposition-v8.4.5.md
```
...
Its inherited floor is established only after the prompt's contact-type pass,
and must retain nonredundant consequences even when their evidence is demoted
from governing rank.
...
specified in `v8.4/spec.md`, using `v8.4.5/run/` as the version root. A full
...
An exact V8.4.5 run is complete only when one persistent formation author wrote
...
morphology, whether ordinary root derivatives remained eligible when their
operation was independently instantiated, whether explicitly conflicting
forms stayed non-governing without being automatically discarded, whether
every demoted consequence was kept, substituted, or responsibly dropped, and
whether any claimed complete formation lost a participant or causal stage.
